...
export function useDevices(): UseDevicesReturn {
  ...
  const [selectedCameraId, setSelectedCameraId] = useLocalStorage<
    string | null
  >("ss_camera_id", null);
  const [selectedMicId, setSelectedMicId] = useLocalStorage<string | null>(
    "ss_mic_id",
    null,
  );
  ...
  const enumerateDevices = useCallback(async () => {
    ...
    try {
      ...
      setState((prev) => {
        ...
        if (nextCamera === prev.camera && nextMic === prev.microphone)
          return prev;
  ...
  const requestMicrophone = useCallback(
    async (deviceId?: string) => {
      setState((prev) => ({
        ...
        microphone: {
          status: "requesting",
          permission: prev.microphone.permission,
        },
      }));
  ...
  const stopCamera = useCallback(() => {
    if (streamsRef.current.camera) {
      ...
      setState((prev) => ({
        ...prev,
        camera: { status: "idle", permission: prev.camera.permission },
      }));
    }
  }, []);
  ...
  const stopMicrophone = useCallback(() => {
    if (streamsRef.current.mic) {
      ...
      setState((prev) => ({
        ...prev,
        microphone: { status: "idle", permission: prev.microphone.permission },
      }));
    }
  }, []);
